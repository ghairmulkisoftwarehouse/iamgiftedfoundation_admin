...
export const Add_PanelUser = (data, toast,navigate) => async (dispatch,) => {
  try {
    ...
    } = await Axios.post(`/admin/admin-user`, data, {
      headers: {
...
export const delete_PanelUser = (id , toast) => async ( dispatch ,) => {
    try {
...
export const update_PanelUser = (id,data, toast,navigate) => async (dispatch) => {
  try {
    ...
    } = await Axios.patch(`/admin/admin-user/${id}`,data, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
