...
const ProgramDetail = ({programDetail,setDetail}) => {
  ...
  const queryClient = useQueryClient();
      ...
      const handleDeletePrograms = async (data) => {
        const title = "Confirm Deletion";
        const message = "Are you sure you want to delete this Program?";
      
        const onYesClick = async () => {
          await dispatch(delete_Programs(data?._id, toast));
  ...
  return (
        ...
        {/* <div
          ...
          >
           ...
           Active
             ...
             <button   
               onClick={()=>handleDeletePrograms(programDetail)}
      ...
      <h2 className=' font-medium text-base md:text-lg leading-7 capitalize'>{programDetail?.title}</h2>
      ...
      <div className=" grid grid-cols-1  gap-4">
        {/* Organizer card */}
           
           {
            ...
            programDetail?.piller  &&(
               <div className="flex flex-col gap-2.5 bg-[#F4F6F6] px-3 py-3 rounded-[15px]">
          <h2 className="font-medium text-sm md:text-base">Piller:</h2>
