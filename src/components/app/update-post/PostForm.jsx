...
import { update_Post, delete_PostImages } from "../../../redux/actions/postActions";
...
const PostForm = () => {
  ...
  const { patchLoading, docDetails } = useSelector((state) => state.post);
  ...
  useEffect(() => {
    if (docDetails) {
      const setInitialData = async () => {
        ...
        if (docDetails.attachments?.length) {
          galleryBase64 = await Promise.all(
            docDetails.attachments.map(async (img) => {
        ...
        setFormData({
          ...
          description: docDetails?.body || "",
          gallery: galleryBase64,
          isFeatured: docDetails?.isFeatured || false,
        });
      };
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      ...(newImages.length > 0 && { attachmentsDataURIs: newImages }),
      isFeatured: formData.isFeatured,
    };

    try {
      await dispatch(update_Post(id, payload, toast, navigate));
      queryClient.invalidateQueries("fetch-all-post");
      queryClient.invalidateQueries("fetch-singlePost");
      resetForm();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="bg-white rounded-[15px] pt-5 pb-11 flex flex-col gap-7 w-full">
          ...
          <div className="sm:col-span-2">
            <MultipleImage
              ...
              onChange={(images, removedImage) => {
                // Delete from server if existing image is removed
                if (removedImage?.isExisting && removedImage?._id) {
                  dispatch(delete_PostImages(removedImage._id, toast));
                }
      ...
      <div className="flex flex-row gap-2 items-center justify-end w-full px-3.5">
        ...
        >
          {patchLoading ? <SubmitLoading size={12} /> : "Update Post"}
        </button>
      </div>
    </div>
  );
