...
import { Add_Pillar } from "../../../redux/actions/pillarActions";
...
const PillerForm = () => {
  ...
  // const { createLoading } = useSelector((state) => state.post);
    const { createLoading } = useSelector(state => state.piller);
...
const handleSubmit = async () => {
  ...
  const payload = {
    ...
    ...(formData.description2 && { description2: formData.description2 }),
  ...
  try {
    await dispatch(Add_Pillar(payload, toast, navigate));
  ...
  return (
   ...
   {/* Description */}
      ...
      <div className="flex flex-row gap-2 items-center justify-end w-full px-3.5">
        ...
        >
          {createLoading ? <SubmitLoading size={12} /> : "Create Piller"}
        </button>
      </div>
    </div>
  );
