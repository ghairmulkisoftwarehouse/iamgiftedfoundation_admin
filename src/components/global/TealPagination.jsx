...
const TealPagination = ({ totalPages, currentPage, setCurrentPage }) => {
  ...
  const renderPageNumbers = () => {
    if (totalPages <= maxPageLinks) {
      ...
      if (currentPage < 3) rightBound = pagesToShow;
      if (currentPage > totalPages - 2) leftBound = totalPages - pagesToShow + 1;

      pageNumbers.push(start);
      if (leftBound > start + 1) pageNumbers.push("...");
      for (let i = leftBound; i <= rightBound; i++) if (i > start && i < end) pageNumbers.push(i);
      if (rightBound < end - 1) pageNumbers.push("...");
      if (totalPages > start) pageNumbers.push(end);
    }
  ...
  // Styling
  const activeBg = "bg-[#9BD6F6]";
  const baseColor = "text-gray-500";
  const activeColor = "text-black  font-bold";
  ...
  const PageItem = ({ pageNum }) => {
    ...
    const pageClasses = isActive ? `${activeBg} ${activeColor}` : `${baseColor} hover:bg-gray-50`;
